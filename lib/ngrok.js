    ...
    try {
      ...
      if (res && res.stdout) {
        ...
        if (data && data.tunnels && data.tunnels.length > 0) {
          // Busca um túnel com protocolo seguro HTTPS para evitar bloqueios de conteúdo misto (Mixed Content)
          const httpsTunnel = data.tunnels.find(t => t.public_url && t.public_url.startsWith('https://'));
          publicUrl = httpsTunnel ? httpsTunnel.public_url : data.tunnels[0].public_url;
          break;
        }
      }
    } catch (err) {
...
export async function stopNgrok() {
  await runSSH(`pkill -f "ngrok http" && sleep 2 || true`);
}
