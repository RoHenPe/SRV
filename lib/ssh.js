...
export async function runSSH(command) {
  ...
  for (const host of hostsToTry) {
    try {
      await ssh.connect({ ...connectConfig, host });
      successfulHost = host;
      cachedHost = host; // Salva no cache para as próximas chamadas
      break;
    } catch (err) {
      console.warn(`[SSH Fallback] Falha ao conectar em ${host}: ${err.message}`);
      lastError = err;
    }
  }
...
export async function apiHandler(fn, request = null, routeName = 'unknown') {
  ...
  try {
    ...
    // Log the request
    if (request) {
      await logRequest(request.method, routeName, token, status, responseTime, err);
    }
  ...
  if (request) {
    const responseTime = Date.now() - startTime;
    await logRequest(request.method, routeName, token, status, responseTime);
  }
}
